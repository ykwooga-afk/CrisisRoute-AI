...
test("temporary diagnostics endpoints are not exposed in the final submission build", async t => {
  const baseUrl = await startServer(t, createServer());

  for (const pathname of [
    "/api/diagnostics/last-gonka-failure",
    "/api/diagnostics/last-public-url-failure"
  ]) {
    const response = await fetch(`${baseUrl}${pathname}`);
    const body = await response.json();
    assert.equal(response.status, 404);
    assert.deepEqual(body, { ok: false, message: "API route not found" });
  }
});
